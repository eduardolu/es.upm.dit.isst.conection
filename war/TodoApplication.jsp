...
<html>
	...
	<body>
		...
		Usted tienes siguientes tareas <c:out value="${fn:length(todos)}" />
		Todos.
		...
		<table>
			...
			<c:forEach items="${todos}" var="todo">
				<tr>
					...
					<td><a class="done"
						href="<c:url value="/done?id=${todo.id}" />">hecho</a></td>
				</tr>
			</c:forEach>
		</table>
			...
			<c:choose>
				<c:when test="${user != null}">
					<form action="/new" method="post" accept-charset="utf-8">
						<table>
							<tr>
								<td><label for="summary">Destino</label></td>
								<td><input type="text" name="summary" id="summary" size="65" /></td>
							</tr>
							<tr>
								<td valign="description"><label for="description">Mensejes</label></td>
